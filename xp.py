...
class Stat():
    def __init__(self, r, name, x, y, l, xp):
        ...
        self.easy_button.grid(column=0, row=1)
        self.hard_button.grid(column=1, row=1)
        self.level = l
        self.xp = xp
        self.next_lvl = get_next_level(self.level)
        self.stats = Label(self.frame, text="Lvl %s, %.1f%% to Lvl %s (%.1f / %.1f xp)" % (
            self.level,
    ...
    def update_text(self):
        self.stats.configure(text="Lvl %s, %.1f%% to Lvl %s (%s / %s xp)" % (
            self.level,
            self.xp / self.next_lvl * 100,
            self.level + 1,
            self.xp,
